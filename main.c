/*
 * Specifications
	The input signal will be a square wave with the amplitude ranging from 0 to +3.3 V,
	generated by a Function Generator – use the SYNC output!
	Your will use the TIM2 general purpose timer to measure the frequency of the input
	signal. Your goal is to determine the number of timer pulses (clock cycles) elapsed
	between two consecutive rising (or falling) edges of the input signal. A current count
	value of the timer pulses is recorded in the TIM2_CNT counter register of TIM2: it
	must be configured to increment every cycle of the TIM2’s clock, whenever TIM2 is
	enabled to count.
	On the STM32F0 Discovery board, you will use the microcontroller’s PA1 I/O pin,
	serving as the EXTI1 external interrupt line, to generate interrupt requests when a
	rising (or falling) edge of the input signal is detected. Your EXTI1 interrupt handler
	will need to access TIM2 (e.g., start/stop the counting process, read TIM2_CNT, etc).
	Your C code must calculate the period and the frequency of the input signal, and then
...
uint16_t  ADC1ConvertedValue = 0, Res = 0;
...
void refresh_LCD(void)
{
	...
	/* 250-ms refresh period (LCD visual perception) */
}
...
void myADC1_Init()
{
	...
	// Step 1:Enable clock for ADC
	RCC->APB2ENR |= RCC_APB2ENR_ADCEN;


	//Step 2: Start ADC calibration:
	ADC1->CR = ADC_CR_ADCAL;
	//wait until the adcal bit is cleared, then the ADC is calibrated
	while(ADC1->CR == ADC_CR_ADCAL);


	//Step 3: configure CFGR1 in continuous mode, in overrun, right data element = 12 bit
	ADC1->CFGR1 |= 0x00003000;


	//Step 4: Select channel 0
	/* Select Channel 0 and Convert it with 239.5 Cycles as sampling time */
	ADC1->CHSELR = ADC_CHSELR_CHSEL0;


	//Step 5: Turn on the ADC
	...
	trace_printf("ADC INITIALIZATION COMPLETED - STARTING CONVERSION\n\n");
	//Start conversion
	ADC1->CR |= ADC_CR_ADSTART;
}
...
void myDAC1_Init()
{
	...
	//Step 2: Clear channel trigger enable bits and buffer
	...
	trace_printf("DAC INITIALIZATION COMPLETED\n\n");
}
...
void LCD_Config(void)
{
	HC595_Setup();
	LCD_Init();
	trace_printf("LCD INITIALIZATION COMPLETED\n\n");
}
